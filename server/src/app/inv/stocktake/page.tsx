...
export default function StocktakeListPage() {
  ...
  const list = trpc.stocktake.list.useQuery({
    page,
    pageSize: 20,
    kind: (kind || undefined) as 'FULL' | 'SAMPLING' | 'DYNAMIC' | undefined,
  ...
  return (
    <AppShell>
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/inv/stocktake/new" data-testid="btn-new-stocktake" className="btn btn-primary">新建盘点单</Link>
      </div>
      ...
      <div className="bg-white rounded shadow border border-gray-100">
        <table data-testid="table-stocktake" className="w-full text-sm">
          ...
          <tbody>
            {(list.data?.items ?? []).map((b) => {
              ...
              return (
                <tr key={String(b.id)} data-testid={`row-stocktake-${b.id}`} className="border-t border-gray-100">
