...
const STATUS_LABEL: Record<number, { text: string; cls: string }> = {
  ...
  25: { text: '待发货', cls: 'badge-warning' },
  30: { text: '已发货', cls: 'badge-success' },
...
export default function OutboundDetailPage() {
  ...
  const id = params.id;
  const detail = trpc.outbound.detail.useQuery({ id });
  ...
  const onSuccess = (text: string) => () => {
    setErr(null);
    setMsg(text);
    utils.outbound.detail.invalidate({ id });
    utils.outbound.list.invalidate();
  };
  const onError = (e: { message: string }) => { setMsg(null); setErr(e.message); };

  const submit = trpc.outbound.submit.useMutation({ onSuccess: onSuccess('已提交审核'), onError });
  const audit = trpc.outbound.audit.useMutation({ onSuccess: onSuccess('审核通过'), onError });
  const ship = trpc.outbound.ship.useMutation({ onSuccess: onSuccess('已发货'), onError });
  const finish = trpc.outbound.finish.useMutation({ onSuccess: onSuccess('已完成'), onError });
  const voidIt = trpc.outbound.void.useMutation({ onSuccess: onSuccess('已作废'), onError });
  ...
  return (
    <AppShell>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">出库单 #{d.docNo}</h1>
      ...
      <div className="bg-white rounded shadow border border-gray-100 p-6 mb-4">
        <div className="grid grid-cols-3 gap-4 text-sm">
          ...
          <Item label="拣货策略" value={d.pickStrategy} />
          <Item label="业务时间" value={new Date(d.operationAt).toLocaleString()} />
          <Item label="目标单号" value={d.targetDocNo ?? '-'} />
      ...
      <div className="bg-white rounded shadow border border-gray-100 mb-4">
        ...
        <table data-testid="table-outbound-lines" className="w-full text-sm">
          ...
          <tbody>
            ...
            {d.lines.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-500">无明细</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <button
          ...
          onClick={() => { if (confirm('确定提交此出库单进入审核？')) submit.mutate({ id }); }}
        >提交</button>
        <button
          data-testid="btn-audit"
          className="btn btn-primary"
          disabled={!canAudit || audit.isPending}
          onClick={() => { if (confirm('确认审核通过？将锁定可用库存。')) audit.mutate({ id }); }}
        >审核</button>
        <button
        ...
        <button
          ...
          onClick={() => { if (confirm('确定发货？将扣减实际库存。')) ship.mutate({ id }); }}
        >发货</button>
        <button
          data-testid="btn-finish"
          className="btn btn-primary"
          disabled={!canFinish || finish.isPending}
          onClick={() => { if (confirm('确定完成此出库单？')) finish.mutate({ id }); }}
        >完成</button>
        <button
          data-testid="btn-void"
          className="btn btn-danger"
          disabled={!canVoid || voidIt.isPending}
          onClick={() => { if (confirm('确定作废此出库单？')) voidIt.mutate({ id }); }}
        >作废</button>
        <button data-testid="btn-back" className="btn btn-secondary" onClick={() => router.push('/inv/outbound')}>返回列表</button>
      </div>
    </AppShell>
  );
}
