...
export default function OutboundListPage() {
  ...
  const list = trpc.outbound.list.useQuery({
    page,
    pageSize: 20,
    kind: (kind || undefined) as 'SALES' | 'USE' | 'TRANSFER' | 'STOCKTAKE' | 'DAMAGE' | 'OTHER' | undefined,
  ...
  return (
    <AppShell>
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/inv/outbound/new" data-testid="btn-new-outbound" className="btn btn-primary">新建出库单</Link>
      </div>
      ...
      <div className="bg-white rounded shadow border border-gray-100">
        <table data-testid="table-outbound" className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-600 text-left">
            <tr>
              ...
              <th className="px-4 py-2">目标单号</th>
          ...
          <tbody>
            {(list.data?.items ?? []).map((b) => {
              ...
              return (
                <tr key={String(b.id)} data-testid={`row-outbound-${b.id}`} className="border-t border-gray-100">
                  ...
                  <td className="px-4 py-2 font-mono">{b.targetDocNo ?? '-'}</td>
