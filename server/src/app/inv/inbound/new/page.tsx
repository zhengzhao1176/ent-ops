...
const KIND_OPTIONS: { value: 'PURCHASE' | 'RETURN' | 'TRANSFER' | 'STOCKTAKE' | 'OTHER'; label: string }[] = [
  { value: 'PURCHASE', label: '采购入库' },
  { value: 'RETURN', label: '退货入库' },
  { value: 'TRANSFER', label: '调拨入库' },
  { value: 'STOCKTAKE', label: '盘盈入库' },
...
function emptyLine(): LineForm {
  return { goodsId: '', locationId: '', qty: '', batchNo: '', unitPrice: '' };
}
...
export default function NewInboundPage() {
  ...
  const [kind, setKind] = useState<'PURCHASE' | 'RETURN' | 'TRANSFER' | 'STOCKTAKE' | 'OTHER'>('PURCHASE');
  const [warehouseId, setWarehouseId] = useState<string>('');
  const [pickStrategy, setPickStrategy] = useState<string>('FIFO');
  const [sourceDocNo, setSourceDocNo] = useState<string>('');
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess('草稿已保存');
      setTimeout(() => router.push('/inv/inbound'), 500);
    } catch (e) {
  ...
  return (
    <AppShell>
      <h1 className="text-xl font-semibold mb-4">新建入库单</h1>
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="bg-white rounded shadow border border-gray-100 p-6 grid grid-cols-2 gap-4 max-w-3xl">
          ...
          <Field label="拣货策略">
            <select data-testid="select-pickStrategy" className="input" value={pickStrategy} onChange={(e) => setPickStrategy(e.target.value)}>
              {PICK_STRATEGIES.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </Field>
          <Field label="来源单号">
            <input data-testid="input-sourceDocNo" className="input" value={sourceDocNo} onChange={(e) => setSourceDocNo(e.target.value)} />
          </Field>
            ...
            <tbody>
              {lines.map((l, idx) => (
                <tr key={idx} data-testid={`row-line-${idx}`} className="border-t border-gray-100">
                  ...
                  <td className="px-2 py-2">
                    <input
                      data-testid={`input-line${idx}-unitPrice`}
                      className="input"
                      value={l.unitPrice}
